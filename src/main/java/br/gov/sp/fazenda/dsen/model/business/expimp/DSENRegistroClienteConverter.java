...
public class DSENRegistroClienteConverter
extends DSGERegistroConverter {
    private static Map<String, String> a = new HashMap();

    protected Registro createBaseRegistro() {
        Registro base = new Registro();
        base.setTagXml("infCliente");
        Campo c = new Campo();
        c.setNome("qtde");
        c.setValueExp("#A");
        base.addCampo(c);
        base.setTipoRegistro("CLIENTE");
        return base;
    }

    protected InputStream getArquivoLayout(String version) {
        return this.getClass().getResourceAsStream(MessageFormat.format(DSENConstants.XML_CLIENTE_LAYOUT, StringHelper.getDigits((String)version)));
    }
    ...
    public static void main(String[] args) {
        try {
            DSENRegistroClienteConverter converter = new DSENRegistroClienteConverter();
            Scanner scanner = new Scanner(converter.getClass().getResourceAsStream("CLIENTE.txt"));
            ...
            registroTOMap.put("E", ClienteTO.class.getCanonicalName());
